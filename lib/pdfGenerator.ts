...
export class AdvancedPDFGenerator {
  ...
  private createHeader(): void {
    ...
    this.doc
      ...
      .text('Phone: +1 234 567 8900 | Email: info@lab.com', { align: 'center' })
      .moveDown(1);
  ...
  private createPatientDetails(patient: PatientData): void {
    ...
    const details = [
      ...
      ['Age/Gender', `: ${patient.pateintAge} ${patient.years_month_day} / ${patient.gender}`],
      ['Blood Group', `: ${patient.bloodGroup || 'N/A'}`],
      ['CNIC', `: ${patient.cnic || 'N/A'}`],
      ['Contact', `: ${patient.patientMobile} ${patient.patientEmail ? `| ${patient.patientEmail}` : ''}`],
      ...
      ['Collection Date', `: ${new Date(patient.createdAt).toLocaleDateString()}`]
    ];
  ...
  private createTestResults(patient: PatientData): void {
    ...
    if (patient.testResults && patient.testResults.length > 0) {
      ...
      headers.forEach((header, i) => {
        this.doc.rect(x, this.doc.y, columnWidths[i], rowHeight).fillAndStroke('#3498db', '#3498db');
        this.doc.text(header, x + 5, this.doc.y + 8, { width: columnWidths[i] - 10, align: 'left' });
        x += columnWidths[i];
      });
    ...
    this.doc
      ...
      .fillColor('#7f8c8d')
      .text('• This report is computer generated and should be interpreted by a qualified medical professional.', 60, this.doc.y, { width: 480 })
      .moveDown(0.3)
      .text('• For any queries regarding this report, please contact the laboratory.', 60, this.doc.y, { width: 480 })
      .moveDown(1);
    ...
    this.doc
      ...
      .font('Helvetica')
      .fillColor('black')
      .text('Lab Technician', 100, signatureY);
  ...
  private createFooter(): void {
    ...
    this.doc
      ...
      .font('Helvetica-Oblique')
      .fillColor('#95a5a6')
      .text(`Report ID: ${Date.now()} | Generated on: ${new Date().toLocaleString()}`, 
